...
from employee.views import DepartmentModelViewSet, EmployeeModelViewSet, AttendanceModelViewSet

urlpatterns = [
    path('V1/departments', DepartmentModelViewSet.as_view({'get': 'list', 'post': 'create'}), name="departments_list_or_create"),
    path(
        'V1/departments/<int:pk>',
        DepartmentModelViewSet.as_view({'get': 'retrieve', 'put': 'update', 'patch': 'partial_update', 'delete': 'destroy'}),
        name="department_get_update_or_delete"
    ),
    ...
    path(
        ...
        name="employee_get_update_or_delete"
    ),
    path(
        'V1/attendance',
